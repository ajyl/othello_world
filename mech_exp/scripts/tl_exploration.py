...
from mech_exp.tl_othello_utils import (
    to_string,
    to_int,
)
...
logits, cache = model.run_with_cache(
    games_int.cuda()[:, :-1], return_type="logits"
)
...
imshow(
    torch.stack(
        [
            torch.stack(
                [
                    probe_out[0, 0, j].argmax(-1),
                    state_stack_one_hot[0, j].argmax(-1).cuda(),
                ]
            )
            for j in range(5, 13)
        ]
...
probe_accs = probe_log_probs.argmax(-1) == state_stack_one_hot.cuda().argmax(
    -1
)
...
imshow(
    torch.stack([even_accs, odd_accs, all_accs]),
    facet_col=0,
    title="Accuracy Per Cell (Mean of 100 Games, Move 5-55)",
    facet_labels=["Odd", "Even", "All"],
)
even_accs_full = probe_accs[0, pos_start:pos_end].mean(0)
odd_accs_full = probe_accs[1, pos_start:pos_end].mean(0)
imshow(
    torch.stack([even_accs_full, odd_accs_full, all_accs]),
    facet_col=0,
    title="Accuracy Per Cell (Mean of 100 Games, Move 5-55)",
    facet_labels=["Odd (Full)", "Even (Full)", "All"],
)
# %%
...
probe_flipped_odd_accs = probe_flipped_odd.argmax(
    -1
) == state_stack_one_hot.cuda().argmax(-1)
...
probe_flipped_even_accs = probe_flipped_even.argmax(
    -1
) == state_stack_one_hot.cuda().argmax(-1)
even_accs_flipped = probe_accs[0, pos_start:pos_end:2].mean(
    0
) + probe_flipped_odd_accs.float().mean(1)[
    0, pos_start + 1 : pos_end : 2
].mean(
    0
)
odd_accs_flipped = probe_accs[1, pos_start + 1 : pos_end : 2].mean(
    0
) + probe_flipped_even_accs.float().mean(1)[1, pos_start:pos_end:2].mean(0)
all_accs = probe_accs[2, pos_start:pos_end].mean(0)
imshow(
    torch.stack([even_accs_flipped / 2, odd_accs_flipped / 2, all_accs]),
    facet_col=0,
    title="Flipped Accuracy Per Cell (Mean of 100 Games, Move 5-55)",
    facet_labels=["Odd (Flipped)", "Even (Flipped)", "All"],
)
...
alternating2 = torch.tensor(
    [1 if i % 2 == 0 else -1 for i in range(59)], device="cuda"
)
...
imshow(
    grid3.T @ grid3,
    x=[f"{lab}{k}" for k in ["(O)", "(E)"] for lab in board_labels],
    y=[f"{lab}{k}" for k in ["(O)", "(E)"] for lab in board_labels],
    title="Cosine Sim of B-W Linear Probe Directions by Cell (Odd Probe)",
    aspect="equal",
)
# %%
...
imshow(
    einsum(
        "components d_model, d_model row col -> components row col",
        stack,
        even_probe_dirs,
    ).reshape(-1, 64)[:, indices],
    y=labels,
    x=[lab for c, lab in enumerate(board_labels) if indices[c].item()],
    title="Contributions to probe for Black Squares",
)
line(
    einsum(
        "components d_model, d_model row col -> components row col",
    ...
    x=np.array(labels),
    line_labels=[
        lab for c, lab in enumerate(board_labels) if indices[c].item()
    ],
    title="Contributions to probe for Black Squares",
)
...
imshow(
    ...
    y=labels,
    x=[lab for c, lab in enumerate(board_labels) if indices[c].item()],
    title="Contributions to probe for White Squares",
)
line(
    einsum(
        "components d_model, d_model row col -> components row col",
...
blank_probe = (
    linear_probe[0, :, :, :, 0]
    - (linear_probe[0, :, :, :, 1] + linear_probe[0, :, :, :, 2]) / 2
)
...
imshow(
    ...
    facet_col=2,
    facet_labels=["Blank", "Black - White"],
    zmin=-2,
    zmax=2,
    title=f"Input Weights for Neuron L5N{ni} via the probe",
)
# %%
for ni in [876, 1016, 561, 1379, 411]:
    x = torch.stack(
        [
            (
                model.blocks[5].mlp.W_in[:, ni][:, None, None]
                / model.blocks[5].mlp.W_in[:, ni].norm()
                * blank_probe
                / blank_probe.norm(0, keepdim=True)
            ).sum(0),
            (
                model.blocks[5].mlp.W_in[:, ni][:, None, None]
                / model.blocks[5].mlp.W_in[:, ni].norm()
                * black_probe
                / black_probe.norm(0, keepdim=True)
            ).sum(0),
        ],
        dim=2,
    )
...
imshow_pos(
    ...
    zmax=0,
    zmin=-6,
    title="Logits at move 20",
    y=[i for i in "ABCDEFGH"],
    x=[i for i in "01234567"],
    aspect="equal",
)
# %%
cell_r = 6
cell_c = 5
flip_dir = (
    linear_probe[0, :, cell_r, cell_c, 2]
    - linear_probe[0, :, cell_r, cell_c, 1]
)
...
flip_state = torch.zeros((64,), dtype=torch.float32, device="cuda") - 10.0
...
imshow_pos(
    torch.stack([state.reshape(8, 8), flip_state.reshape(8, 8)]),
    zmax=0,
    zmin=-6,
    title="Logits pre and post intervening",
    y=[i for i in "ABCDEFGH"],
    x=[i for i in "01234567"],
    aspect="equal",
    facet_col=0,
    facet_labels=["Original", "Flipped"],
)
# %%
pos = 50
game_index = 8
...
print(
    cache["resid_post", 6][game_index, pos]
    @ linear_probe[0, :, cell_r, cell_c]
)
# %%
...
for scale in scales:
    ...
    flipped_logits = model.run_with_hooks(
        games_int[game_index : game_index + 1, : pos + 1],
        fwd_hooks=[
            #  ("blocks.3.hook_resid_post", flip_hook),
...
scatter(
    y=state_big,
    x=flip_state_big,
    title=f"Original vs Flipped {str_to_label(8*cell_r+cell_c)} at Layer {layer}",
    xaxis="Flipped",
    yaxis="Original",
    hover=board_labels,
    facet_col=0,
...
flip_dir_2 = (
    linear_probe[0, :, cell_r_2, cell_c_2, 2]
    - linear_probe[0, :, cell_r_2, cell_c_2, 1]
)
print(cache["resid_post", 6][game_index, pos] @ flip_dir)
print(
    cache["resid_post", 6][game_index, pos]
...
scatter(
    y=state_big,
    x=flip_state_big,
    title=f"Original vs Flipped {str_to_label(8*cell_r+cell_c)} & {str_to_label(8*cell_r_2+cell_c_2)} at Layer {layer}",
    xaxis="Flipped",
    yaxis="Original",
    hover=board_labels,
    facet_col=0,
...
linear_probe = torch.load(
    "/workspace/othello_world/linear_probe_L4_blank_vs_color_v1.pth"
)
mode = 0
blank_probe_old = (
    linear_probe_old[mode, :, :, :, 0]
    - linear_probe_old[mode, :, :, :, 1] * 0.5
    - linear_probe_old[mode, :, :, :, 2] * 0.5
)
next_probe_old = (
    linear_probe_old[mode, :, :, :, 2] - linear_probe_old[mode, :, :, :, 1]
)
...
blank_probe[:, 3:5, 3:5] = 0.0
next_probe = -next_probe / next_probe.norm(dim=0, keepdim=True)
# %%
imshow(
    (blank_probe_old / blank_probe_old.norm(dim=0, keepdim=True))
    .reshape(512, 64)
    .T
    @ (blank_probe / blank_probe.norm(dim=0, keepdim=True)).reshape(512, 64),
    title="Old Blank Probe",
    x=board_labels,
    y=board_labels,
)
imshow(
    (next_probe_old / next_probe_old.norm(dim=0, keepdim=True))
    .reshape(512, 64)
    .T
    @ (next_probe / next_probe.norm(dim=0, keepdim=True)).reshape(512, 64),
    title="Old Blank Probe",
    x=board_labels,
    y=board_labels,
)
...
# %%
imshow(
    (blank_probe / blank_probe.norm(dim=0, keepdim=True)).reshape(512, 64).T
    @ (blank_probe / blank_probe.norm(dim=0, keepdim=True)).reshape(512, 64),
    title="Old Blank Probe",
    x=board_labels,
    y=board_labels,
...
imshow(
    (next_probe / next_probe.norm(dim=0, keepdim=True)).reshape(512, 64).T
    @ (next_probe / next_probe.norm(dim=0, keepdim=True)).reshape(512, 64),
    title="Old Blank Probe",
    x=board_labels,
    y=board_labels,
...
def patching_hook(resid_post, hook, row, col, scale=1):
    resid_post[0, -1, :] -= (
        (resid_post[0, -1, :] @ next_probe[:, row, col])
        * next_probe[:, row, col]
        * (scale + 1)
    )
    return resid_post
...
newly_legal = [
    str_to_label(move)
    for move in flipped_valid_moves
    if move not in valid_moves
]
newly_illegal = [
    str_to_label(move)
    for move in valid_moves
    if move not in flipped_valid_moves
]
...
U_probe, S_probe, Vh_probe = (
    torch.cat([blank_probe, next_probe], dim=1).reshape(512, 128).svd()
)
U_blank = U_blank[:, :-4]
U_probe = U_probe[:, :-4]
...
for label, U in [
    ...
    for act_name in ["attn_out", "mlp_out", "resid_pre"]:
        frac_retained = (
            torch.stack(
                [
                    (((big_cache[act_name, layer])) @ U).norm(dim=-1)
                    / ((big_cache[act_name, layer])).norm(dim=-1)
...
imshow(
    ...
    yaxis="Mode",
    xaxis="Layer",
    title="Average Fraction of Residual Captured by Blank + Next",
    color_continuous_midpoint=0.125,
    zmax=1,
    zmin=-0.75,
    y=out_labels,
)
...
fig = line(
    [
        (model.W_E @ U).pow(2).sum(dim=-1) / model.W_E.pow(2).sum(dim=-1)
        for U in [U_blank, U_next, U_probe]
    ],
    return_fig=True,
    line_labels=["blank", "next", "blank + next"],
    title="Embed vs Probe Subspaces",
    range_y=(0, 1),
)
...
fig = line(
    [
        (model.W_U.T @ U).pow(2).sum(dim=-1) / model.W_U.T.pow(2).sum(dim=-1)
        for U in [U_blank, U_next, U_probe]
    ],
    return_fig=True,
    line_labels=["blank", "next", "blank + next"],
    title="Unembed vs Probe Subspaces",
    range_y=(0, 1),
)
fig.add_hline(y=0.25)
fig.show()
...
fig = line(
    [lis.sort().values for lis in line_list],
    return_fig=True,
    line_labels=label_list,
    title="W_in vs Probe Subspaces",
    range_y=(0, 1),
...
acc_blank = (
    (probe_out[0].argmax(-1) == big_state_stack_one_hot[0].argmax(-1))
    .float()
    .mean()
)
acc_color = (
    (
        (next_probe_out[:, 5:-5] > 0).int()
        == big_state_stack_one_hot[1, :, 5:-5].argmax(-1)
    )
    * big_state_stack_one_hot[1, :, 5:-5].sum(-1)
).float().sum() / (big_state_stack_one_hot[1, :, 5:-5]).float().sum()
print(acc_blank)
print(acc_color)
# %%
imshow(
    [
        big_states[50, 30]
        * (big_state_stack_one_hot[1, 50, 30].argmax(-1) - 0.5),
        big_states[50, 30] * (probe_out[1, 50, 30].argmax(-1) - 0.5),
    ],
    facet_col=0,
)
imshow(
    [
        (next_probe_out[50, 30] > 0).int()
        * (big_state_stack_one_hot[1, 50, 30].argmax(-1) - 0.5),
        big_states[50, 30] * (probe_out[1, 50, 30].argmax(-1) - 0.5),
    ],
    facet_col=0,
...
imshow(
    [
        big_states[50, 31]
        * (big_state_stack_one_hot[1, 50, 31].argmax(-1) - 0.5),
        big_states[50, 31] * (probe_out[1, 50, 31].argmax(-1) - 0.5),
    ],
    facet_col=0,
)
...
def get_acc_next(tensor):
    ...
    return 0.5 * (
        (
            (
                (next_probe_out > 0)[:, 2:-2:2] & (big_states == -1)[:, 2:-2:2]
            ).sum()
            + (
                (next_probe_out < 0)[:, 2:-2:2] & (big_states == 1)[:, 2:-2:2]
            ).sum()
        )
        / (big_states != 0)[:, 2:-2:2].sum()
        + (
            (
                (next_probe_out > 0)[:, 3:-2:2] & (big_states == 1)[:, 3:-2:2]
            ).sum()
            + (
                (next_probe_out < 0)[:, 3:-2:2] & (big_states == -1)[:, 3:-2:2]
            ).sum()
        )
        / (big_states != 0)[:, 3:-2:2].sum()
    )
...
def get_acc_blank(tensor):
    ...
    return (
        (blank_probe_out < 0)[:, 2:-2] & (big_states == 0)[:, 2:-2]
    ).float().mean() + (
        (blank_probe_out >= 0)[:, 2:-2] & (big_states != 0)[:, 2:-2]
    ).float().mean()
...
line(
    [
        [get_acc_next(big_cache[act_name, layer]) for layer in range(8)]
        for act_name in ["attn_out", "mlp_out", "resid_post"]
    ],
    title="Accuracy of Next Move Prediction",
    line_labels=["attn_out", "mlp_out", "resid_post"],
    range_y=(0.0, 1),
)
line(
    [
        [get_acc_blank(big_cache[act_name, layer]) for layer in range(8)]
        for act_name in ["attn_out", "mlp_out", "resid_post"]
    ],
    title="Accuracy of Blank Move Prediction",
    line_labels=["attn_out", "mlp_out", "resid_post"],
    range_y=(0.0, 1),
)
# %%
line(
    ...
    title="Accuracy of Blank Move Prediction per layer 0 head",
    range_y=(0.0, 1),
)
...
vec /= vec.norm()
line(
    [vec @ blank_probe.reshape(512, 64), vec @ next_probe.reshape(512, 64)],
    x=np.array(board_labels),
    title="Blank and Next Probe for Layer 5, Neuron 1393",
)
# %%
W_in = einops.rearrange(
    model.W_in, "layer d_model neuron -> (layer neuron) d_model"
)
neuron_weights_shifted = W_in @ torch.cat(
    [blank_probe.reshape(512, 64), next_probe.reshape(512, 64)], dim=1
)
...
def plot_neuron(ni, layer=0):
    ...
    imshow(
        [blank_board, next_board],
        facet_col=0,
        title=f"Blank and Next Probe for Layer {layer}, Neuron {ni}",
        facet_labels=["Blank", "Next"],
    )
...
imshow(
    [
        (vec[:, None, None] * next_probe_old_norm).sum(0),
        (vec[:, None, None] * next_probe).sum(0),
    ],
    facet_col=0,
)
blank_probe_old_norm = blank_probe_old / blank_probe_old.norm(
    dim=0, keepdim=True
)
imshow(
    [
        (vec[:, None, None] * blank_probe_old_norm).sum(0),
        (vec[:, None, None] * blank_probe).sum(0),
    ],
    facet_col=0,
)
# %%
...
neuron_weights_shifted = W_in @ torch.cat(
    [
        blank_probe_old_norm.reshape(512, 64),
        next_probe_old_norm.reshape(512, 64),
    ],
    dim=1,
)
neuron_labels = [f"L{l}N{n}" for l in range(8) for n in range(model.cfg.d_mlp)]
...
imshow(
    values[:100, :],
    hover=indices[:100, :],
    x=[f"{lab} {t}" for t in ["blank", "next"] for lab in board_labels],
)
imshow(
    values[-100:, :],
    hover=indices[-100:, :],
...
sorted_neuron_acts, top_board_indices = neuron_acts.flatten().sort(
    descending=True
)
...
line(
    sorted_neuron_acts[:cutoff],
    title=f"Top {cutoff} Neuron activations for L{index // 2048}N{index % 2048}",
)
imshow(
    [abs_state_top, alt_state_top],
    facet_col=0,
    facet_labels=["Absolute", "Alternating"],
)
# %%
