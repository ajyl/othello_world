...
class GPTforProbing(GPT):
    def __init__(self, config, probe_layer=-1, ln=False):
        ...
        assert (
            self.probe_layer <= self.n_layer and self.probe_layer >= 0
        ), "Invalid layer index to probe"
        self.ln = ln
...
class GPTforIntervention(GPT):
    def __init__(self, config, probe_layer=-1):
        ...
        self.probe_layer = self.n_layer if probe_layer == -1 else probe_layer
        assert (
            self.probe_layer <= self.n_layer and self.probe_layer >= 1
        ), "Invalid layer index to probe"
