...
class BatteryProbeClassification(nn.Module):
    ...
    def __init__(
        self, device, probe_class, num_task, input_dim=512
    ):  # from 0 to 15
        super().__init__()
        ...
        self.proj = nn.Linear(
            self.input_dim, self.probe_class * self.num_task, bias=True
        )
    ...
    def forward(self, act, y=None):
        # [B, f], [B, #task]
        logits = self.proj(act).reshape(
            -1, self.num_task, self.probe_class
        )  # [B, #task, C]
        if y is None:
            return logits, None
...
class BatteryProbeClassificationTwoLayer(nn.Module):
    # combines 64 classification problem for the case of Othello
    def __init__(
        self, device, probe_class, num_task, mid_dim, input_dim=512
    ):  # from 0 to 15
        super().__init__()
        self.input_dim = input_dim
        self.probe_class = probe_class
        ...
        self.proj = nn.Sequential(
            ...
            nn.ReLU(True),
            nn.Linear(
                self.mid_dim, self.probe_class * self.num_task, bias=True
            ),
        )
        self.apply(self._init_weights)
        self.to(device)
