...
class Trainer:
    ...
    def flush_plot(
        ...
        fig, axs = plt.subplots(
            1, 2, figsize=(20, 10), dpi=80, facecolor="w", edgecolor="k"
        )
        axs = axs.flat
    ...
    def save_traces(
        ...
        with open(
            os.path.join(self.config.ckpt_path, "tensorboard.txt"), "w"
        ) as f:
    ...
    def save_checkpoint(self):
        ...
        torch.save(
            ...
            os.path.join(self.config.ckpt_path, "checkpoint.ckpt"),
        )
    ...
    def train(self, prt=True):
        ...
        def run_epoch(split):
            ...
            losses = []
            totals_epoch = np.zeros(
                60, dtype=float
            )  # np.array of shape [60], for positions of age 0 to 59
            hits_epoch = np.zeros(
                60, dtype=float
            )  # np.array of shape [60], for positions of age 0 to 59
            pbar = (
                tqdm(enumerate(loader), total=len(loader), disable=not prt)
                if is_train
            ...
            for it, (x, y, age) in pbar:
                ...
                with torch.set_grad_enabled(is_train):
                    ...
                    totals_epoch += np.array(
                        [torch.sum(age == i).item() for i in range(60)]
                    ).astype(float)
                    y_hat = torch.argmax(
                ...
                if is_train:
                    ...
                    mean_acc = (
                        np.sum(hits_epoch).item() / np.sum(totals_epoch).item()
                    )
                    lr = optimizer.param_groups[0]["lr"]
                    pbar.set_description(
                        f"epoch {epoch+1}: train loss {mean_loss:.5f}; lr {lr:.2e}; train acc {mean_acc*100:.2f}%"
                    )
            if is_train:
                self.train_loss_cont.append(mean_loss)
                self.train_acc_cont.append(mean_acc)
                self.train_strat_acc_cont.append(
                    (hits_epoch / totals_epoch).tolist()
                )
            ...
            if not is_train:
                ...
                self.test_strat_acc_cont.append(
                    (hits_epoch / totals_epoch).tolist()
                )
                return test_loss
